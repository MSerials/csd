...
CDahengCam::CDahengCam(HWND hWnd)
	: m_nGray(0)
	, m_nRoiX(0)
	, m_nRoiY(0)
	, m_nRoiH(0)
...
void __stdcall CDahengCam::OnFrameCallbackFun(GX_FRAME_CALLBACK_PARAM *pFrame){
	...
	if (pFrame->status == 0)
	{
		...
		else
		{
			...
			pDlg->DrawImg(pDlg->m_pImgBuffer);
			g.g_evtGetVideoCapture.SetEvent();
			//Sleep(pDlg->m_ContinueSnapDelay);
		}
	}
...
//相机流程
int CDahengCam::MainFunction(HWND hwnd)
{

	if (NULL == hwnd){
		AfxMessageBox(L"未能获取句柄");
		return -1;
	}
	m_hwnd = hwnd;
